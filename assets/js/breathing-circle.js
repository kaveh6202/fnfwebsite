class BreathingCircle {
    constructor(breathingType = 'kamBreathing') {
        ...
        // Constants for circle creation
        this.STROKE_WIDTH = 3;
        this.SAFETY_PADDING = 3; // extra pixels to avoid visual clipping
        this.RADIUS_JITTER_MAX = 10; // px randomness per circle
        this.MAX_SCALE = 2;
    ...
    init() {
        ...
        // No breathing pattern controls UI - removed as requested
    ...
    createCircles() {
		...
		const maxRadius = (this.svgSize / 2) - (this.STROKE_WIDTH / 2) - this.SAFETY_PADDING - this.RADIUS_JITTER_MAX;
		const baseRadius = Math.max(1, Math.floor((maxRadius - orbitRadius) / this.MAX_SCALE));
        ...
        for (let i = 0; i < circleCount; i++) {
			...
			const rOffset = Math.round((Math.random() * (this.RADIUS_JITTER_MAX * 2)) - this.RADIUS_JITTER_MAX);
        ...
        window.addEventListener('resize', () => {
			...
			const maxR = (this.svgSize / 2) - (this.STROKE_WIDTH / 2) - this.SAFETY_PADDING - this.RADIUS_JITTER_MAX;
            this.orbitRadius = Math.max(6, Math.min(12, Math.floor(this.svgSize * 0.02)));
            this.baseRadius = Math.max(1, Math.floor((maxR - this.orbitRadius) / this.MAX_SCALE));
        ...
        const step = (now) => {
            ...
            if (desiredCount !== this.circles.length) {
                ...
                for (let i = 0; i < desiredCount; i++) {
                    ...
                    circle.setAttribute('cx', String(this.centerX));
                    circle.setAttribute('cy', String(this.centerY));
                    const off2 = Math.round((Math.random() * (this.RADIUS_JITTER_MAX * 2)) - this.RADIUS_JITTER_MAX);
